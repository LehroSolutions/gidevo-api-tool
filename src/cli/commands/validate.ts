...
export async function validateCommand(spec: string, options: { strict?: boolean }) {
  ...
  ui.sectionHeader('SCHEMA INTEGRITY VERIFICATION');

  // Check if spec file exists
  if (!fs.existsSync(spec)) {
    ui.error('Source Not Found', ui.filePath(spec));
    ui.info('Resolution', 'Verify file path and ensure source exists');
    process.exit(1);
  }
  ...
  ui.table(
    ['Parameter', 'Value'],
    [
      ['Source Document', ui.filePath(specPath)],
      ['Verification Mode', ui.highlight(mode.toUpperCase())],
    ]
  );

  telemetry.track('validate_start', { mode });

  const spinner = await createSpinner(`Analyzing schema integrity...`);
  ...
  try {
    ...
    if (result.valid) {
      ui.success('Schema Integrity Verified', `Scan completed in ${formatDuration(duration)}`);
      ...
      if (ext === '.yaml' || ext === '.yml' || ext === '.json') {
        try {
          ...
          if (parsed.openapi) {
            ui.sectionHeader('DOCUMENT METADATA');
      ...
      ui.error('Integrity Check Failed', `Detected ${result.errors.length} anomaly(ies)`);

      ui.sectionHeader('ANOMALY REPORT');
    ...
    spinner.stop();
    ui.error('Verification Aborted', error.message);
