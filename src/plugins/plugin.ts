...
import { filenameFromMetaUrl } from '../core/runtime.js';
...
export async function loadPlugins(pluginDir: string): Promise<Plugin[]> {
  ...
  const requireFromLoader = createRequire(currentFile);
  ...
  for (const file of fs.readdirSync(realDir)) {
    ...
    try {
      const mod = await loadPluginModule(realPluginPath, requireFromLoader);
...
async function loadPluginModule(pluginPath: string, requireFromLoader: NodeRequire): Promise<any> {
  try {
    ...
    if (
      ...
      return requireFromLoader(pluginPath);
    }
    throw error;
  }
}
