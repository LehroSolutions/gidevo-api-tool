...
export interface CheckedOutputPath extends PreparedOutputDirectory {
  exists: boolean;
}
...
export function checkOutputPath(
  ...
  if (exists && fs.lstatSync(resolvedOutput).isSymbolicLink()) {
    throw new Error(`Symlink detected in output path: ${resolvedOutput}`);
  }

  const outputPath = exists ? fs.realpathSync(resolvedOutput) : resolvedOutput;

  if (!outsideAllowed && !isPathWithin(projectRoot, outputPath)) {
    throw new Error('Symlink traversal detected in output directory.');
  }
