...
function escapeCodeLiteral(value: string): string {
  return value
    .replace(/\\/g, '\\\\')
    .replace(/'/g, "\\'")
    .replace(/\r/g, '\\r')
...
export function registerHandlebarsHelpers(): void {
  ...
  Handlebars.registerHelper('resolveType', function (schema): string {
    ...
    if (schema.type === 'string') {
      ...
      if (schema.enum) {
        return schema.enum.map((e: string) => `'${escapeCodeLiteral(String(e))}'`).join(' | ');
      }
      return 'string';
    }
  ...
  Handlebars.registerHelper('resolveGoType', function (schema, requiredList, propName): string {
  ...
  Handlebars.registerHelper('goFieldName', function (name: string): string {
