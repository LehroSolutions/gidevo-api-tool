...
- Package output is ESM-first.
- CommonJS `require('gidevo-api-tool')` is no longer the primary programmatic interface.

#### New

- Added `doctor` for read-only project health checks.
- Added `workflow` for validate-then-generate runs.
- Added JSON output for `doctor` and `workflow`.
- Added `createProgram()` and `run()` exports for embedding and tests.

#### CLI UX

- Replaced default "neural/synthesis" CLI vocabulary with direct developer terminology.
- Centralized `--quiet`, `--no-color`, and `--no-spinner` behavior.
- Replaced legacy Inquirer usage with `@inquirer/prompts`.

#### Runtime and Tooling

- Migrated TypeScript output to NodeNext/ESM.
- Updated CLI/runtime packages to current majors.
- Updated CI matrix to Node 24 LTS and Node 26 Current.
- Updated CodeQL workflow to v4 actions.

#### Verification

- Added command tests for `doctor`, `workflow`, and `createProgram()`.
- Added E2E coverage for `doctor --json`, `workflow --dry-run`, and Go workflow generation.
