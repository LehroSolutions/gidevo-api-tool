...
describe('validateCommand', () => {
  ...
  it('exits with error for file not found', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Specification not found'));
  });

  it('valid spec logs valid', async () => {
    await validateCommand(fixtureValid, {});
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('SPECIFICATION VALIDATION'));
  ...
  it('invalid spec exits with error', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Specification is invalid'));
  });
});
