...
describe('CLI Generate Command', () => {
  ...
  it('exits with error for missing spec', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Missing specification'));
    exitSpy.mockRestore();
  });
  ...
  it('exits with error for unsupported language', async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Unsupported language'));
    exitSpy.mockRestore();
  });
});
